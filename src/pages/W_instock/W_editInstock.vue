                    ...
                    <el-form label-width="120px" :rules="rules" :model="instockForm" ref="instockForm" style="width: 100%;">
                        <el-row>
                            ...
                            <el-col :span="10">
                                <el-form-item label="入库店铺：">
                                    <el-select v-model="instockForm.waimaistore_id" @change="clear" placeholder="请选择入库店铺" class="col">
                                        <el-option
                                                v-for="item in store"
                        ...
                        <el-row>
                            <el-form-item label="入库类型：">
                                <el-col :span="12">
                                    <el-select v-model="instockForm.type_id" placeholder="请选择入库类型" clearable style="width: 40%;">
                                        <el-option
                                    ...
                                    <el-select v-if="instockForm.type_id==1" v-model="purchase" @change="getAllDistribute" placeholder="配送单号" clearable style="width: 40%;">
                                        <el-option
                                                v-for="item in order"
                                                :label="item.ob_bn"
                                                :value="item.ob_id"
...
<script>
    import { getSkuList, getCategoriesList, getWinstockType, WinstockDetail, WstoreList, WinstockEdit, getOrderBlanks, getDetailsBlanks} from '../../api/api'
    export default {
        data () {
            return {
                cat_id: '',
...
//          获取分类列表
            ...
            WinstockDetail(this.$route.query.instock_id).then((res)=>{
                ...
                this.purchase = parseInt(res.data.post_bn)
            })
        },
        methods: {
            getback(){
                this.$router.push({path: "/waimai_stocks/instocks/lists",query:{criteria: this.$route.query.criteria}})
            },
            clear(){
                if(this.bools){
                    this.bools = false
...
//                                alert(11)
            ...
            subMit(){
                ...
                WinstockEdit(this.$route.query.instock_id,params).then((res)=>{
