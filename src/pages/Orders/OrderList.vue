<template>
    <section>
        ...
        <el-table
            ...
            <el-table-column prop="receiver_address" label="收货地址" min-width="150">
                <template slot-scope="props">
                    <elliptical :message="props.row.receiver_address" :length="8"></elliptical>
                </template>
            </el-table-column>
            <el-table-column
                    prop="total_price"
...
<script>
    import {getOrderList, storeList} from "../../api/api"
    export default{
        data() {
            return {
                pickerOptions: {
        ...
        methods: {
            ...
            educe(){
                window.open(`${this.$store.state.urlBackstage}/orders/export?access_token=${sessionStorage.getItem('access_token')}&status=${this.status}&store_id=${this.store_id}&startTime=${this.start_date}&endTime=${this.end_date}&platform=${this.platform}&sort_type=DESC&sort_by=created_time`)
            },
            reLoad(){
                this.value = "";
                this.store_id = "";
            ...
            details(index){
                ...
                this.$router.push({
                    path: '/orderDetails',
                    query: {order_id: this.tableData[index].tid, normal: "y", obj: obj}
                })
            },
