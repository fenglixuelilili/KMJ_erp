    ...
    <section class='bg' id="gg">
        ...
        <div class="shadow">
            ...
            <el-row  class='bg' style="margin-top: 10px;">
                <el-col :span='21'>
                    <el-form label-width="120px" :rules="rules" :model="outstockForm" ref="outstockForm" style="width: 100%;">
                        <el-row>
                            <el-col :span="8">
                            ...
                            <el-col :span="8">
                                <el-form-item label="出库类型：">
                                    <el-select v-model="outstockForm.type_id" placeholder="请选择出库类型" clearable class="col">
                                        <el-option
                        ...
                        <el-row>
                            ...
                            <el-col :span="8" v-if="outstockForm.type_id==4">
                                <el-form-item label="入库仓库：">
                                    <el-select v-model="outstockForm.original_id" clearable placeholder="请选择" class="col">
                                        <el-option
                                                v-for="item in options"
                                                :label="item.warehouse_name"
                                                :value="item.warehouse_id"
            ...
            <el-row class='bg'>
                <el-col :span='22' :offset="1">
                    <div style='padding: 10px;padding-left: 0;'>
                        ...
                        <el-button type="primary" style='width: 150px;' @click='educe' v-if="outstockForm.warehouse_id">导出标准表格</el-button>
                        <el-upload
                                ...
                                :on-success="getInfo"
                                v-if="outstockForm.warehouse_id"
                                name="excel"
                                accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet">
                            <el-button type="primary" style='width: 150px;margin-left: 10px;'>导入标准表格</el-button>
                        </el-upload>
                    </div>
                    <el-table
                            :empty-text="text"
                        ...
                        <el-table-column label="出库数量" min-width="95">
                            <template slot-scope="props">
                        ...
                        <el-table-column label="运费(选填)" width="120">
                            <template slot-scope="props">
                                <input v-model="props.row.post_fee" style="width: 99%;" class="el-input__inner">
                            </template>
                        </el-table-column>
        ...
        <el-row class="bg" style='text-align: center;margin: 0;'>
            ...
            <el-button type="primary" style='width: 200px;margin: 20px;background:#FFF;color: black;border: 1px solid #ccc' @click='$router.replace("/iostocks/outstocks/manager/list")'>取消</el-button>
        </el-row>
    </section>
</template>
<script>
    import { getPredefined, getSkuList, addOutstock, getWareHousesList, getCategoriesList, getInstockType, getOutstocksDetail } from '../../api/api'
    export default {
        data() {
            return {
                filters: {
                ...
                action: this.$store.state.urlBackstageApi + "/outstocks/importInputExcel?access_token=" + sessionStorage.getItem('access_token')
            }
        },
        created () {
            var parm = {
            ...
            getPredefined(parm).then((res)=>{
                this.outstockForm.outstock_bn = res.data.bn
            })
//            出库类型列表
...
//            获取仓库列表
            getWareHousesList().then(res => {
                ...
                if (this.$route.query.warehouse_id){
                    this.outstockForm.warehouse_id = this.$route.query.warehouse_id;
...
//            获取分类列表
        ...
        methods: {
            ...
            educe(){
                window.open(`${this.$store.state.urlBackstageApi}/outstocks/exportInputExcel?access_token=${sessionStorage.getItem('access_token')}&warehouse_id=${this.outstockForm.warehouse_id}`)
            },
            getInfo(res){
                console.log(res)
                this.tableDataI = res.data;
            },
            btnS(){
                this.tableDataI = [];
                this.cat_ids = [];
