...
<script>
    import { WstoreList, editBrolerage } from '../../api/api'
    export default {
        data () {
            return {
                tableDataI: [],
        ...
        created () {
            if (this.$route.query.data){
                ...
                this.normal = data.common_commission;
                this.brokerage = data.activity_commission;
                this.tableDataI = data.stores;
                this.memo = data.commission_memo;
            }
            this.getSkusList();
        },
        methods: {
            getback(){
                this.$router.push({path: "/store_commission/list",query:{criteria: this.$route.query.criteria}})
            },
            subMit(){
                var parm = {
                ...
                parm.storeIds = str;
                editBrolerage(this.$route.query.data.commission_group,parm).then((res)=>{
                    if (!res.error_code){
                        this.$notify({
                            title: '成功',
                            message: '佣金设置修改成功',
