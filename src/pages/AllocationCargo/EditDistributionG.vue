                        ...
                        <el-row>
                            <el-col :span="8">
                                <el-form-item label="出货仓库：">
                                    <el-select v-model="house" filterable clearable placeholder="请选择仓库" @change="clearI" class="col">
                                        <el-option
...
<script>
    import { storeList, getwarehouse, getItemsList, getDetailsBlanks, editBlanks, getCategoriesList } from '../../api/api'
    export default {
        data () {
            return {
                defaultProps: {
        ...
        created () {
            ...
            var parms = { ob_id:this.$route.query.ob_id }
            getDetailsBlanks(this.$route.query.ob_id,parms).then((res)=>{
                this.tableDataI = res.data.items;
                this.shop = res.data.store.waimaistore_id;
                this.house = res.data.warehouse_id;
                this.operator = res.data.operator;
                this.ob_bn = res.data.ob_bn;
                this.ob_id = res.data.ob_id;
                this.warehouse_id = res.data.warehouse_id;
                for (let i = 0;i < this.tableDataI.length;i++){
                    this.tableDataI[i].sku_store = res.data.items[i].store;
                }
                var stringTime = res.data.start_date;
                var timestamp = Date.parse(new Date(stringTime));
                var newDate = new Date();
                newDate.setTime(timestamp);
                this.values = newDate.toString();
        ...
        methods:{
            ...
            subMit(){
                ...
                editBlanks(this.$route.query.ob_id,parm).then((res)=>{
                    this.$notify.info({
                        message: res.message
                    });
