    ...
    <section>
                        ...
                        <el-row>
                            <el-col :span="8">
                                <el-form-item label="出库单号：" class="hspace">
                                    <el-input v-model="filters.instock_bn" placeholder="出库单号" class="col"
                                              :size="size"></el-input>
                                </el-form-item>
                            </el-col>
                            <el-col :span="8">
                                <el-form-item label="出库店铺：" class="hspace">
                                    <el-select v-model="filters.waimaistore_id" filterable clearable placeholder="出库店铺"
                                               class="col" :size="size">
                                        <el-option
                                                v-for="item in store"
                            ...
                            <el-col :span="8">
                                <el-form-item label="出库状态：" class="hspace">
                                    <el-select v-model="filters.in_status" clearable placeholder="出库状态" class="col"
                                               :size="size">
                                        <el-option
                        ...
                        <el-row v-if="$store.state.isPackUp">
                            <el-col :span="8">
                                <el-form-item label="出库类型：" class="hspace">
                                    <el-select v-model="filters.instockType" clearable placeholder="出库类型" class="col"
                                               :size="size">
                                        <el-option
                        ...
                        <el-row v-if="$store.state.isPackUp">
                            ...
                            <el-col :span="8">
                                <el-form-item label="平台单号：">
                                    <el-input v-model="filters.tid" placeholder="平台单号" class="col"
                                              :size="size"></el-input>
                                </el-form-item>
                            </el-col>
                            <el-col :span="8">
                                <el-form-item label="来源平台：">
                                    <el-select v-model="filters.platform" clearable placeholder="来源平台" class="col"
                                               :size="size">
                                        <el-option
        ...
        <template>
            <el-table :data="instockList" border highlight-current-row v-loading="listLoading" style="width: 100%;"
                ...
                <el-table-column inline-template :context="_self" label="操作" width="170">
                    <template>
                        <p style="margin: 5px 0">
                            <el-button size="mini" @click="copy(row)">复制</el-button>
                        ...
                        <p style="margin: 5px 0">
                            ...
                            <el-button type="primary" size="mini" @click="confirm(row)"
                                       v-if="row.confirm_status == 1&&row.out_status!=2">确认
                            </el-button>
                ...
                <el-table-column prop="outstock_bn" label="出库单号" min-width="150">
                </el-table-column>
                <el-table-column prop="tid" label="平台单号" min-width="150">
                </el-table-column>
                <el-table-column prop="store_name" label="出库店铺" min-width="150">
                </el-table-column>
                <el-table-column label="出库类型" :formatter="instockTypes" min-width="85">
                </el-table-column>
                <el-table-column label="出库状态" min-width="85">
...
<script>
    import {getWinstockType, WstoreList, getWoutstockList, WconfirmOutstock, WdeleteOutstock} from '../../api/api';
    export default {
        data() {
            return {
                pickerOptions: {
                ...
                platform: [{label: '京东到家', value: 'jddj'}, {label: '美团外卖', value: 'meituan'}, {
                    label: '百度外卖',
                    value: 'bd'
        ...
        created() {
            ...
            if (this.$route.query.criteria) {
                ...
                this.filters.tid = criteria.tid;
                this.filters.platform = criteria.platform;
                this.filters.instockType = criteria.type_id;
                this.filters.instock_bn = criteria.outstock_bn;
                this.filters.in_status = criteria.out_status;
                this.filters.waimaistore_id = criteria.store_id;
                this.filters.platform_bn = criteria.platform_bn;
                ...
                this.confirmed_start_date = criteria.confirmed_start_date;
        ...
        methods: {
            ...
            printInstock(val){
                window.open(`${this.$store.state.url}/W_outstock.html?${val.outstock_id}?access_token=${sessionStorage.getItem('access_token')}`)
            },
            detail(val){
                let criteria = {
                    page: this.page,
                    ...
                    outstock_bn: this.filters.instock_bn,
                    out_status: this.filters.in_status,
                    store_id: this.filters.waimaistore_id,
                    platform_bn: this.filters.platform_bn,
                    ...
                    confirmed_start_date: this.confirmed_start_date,
                    confirm_end_date: this.confirm_end_date,
                }
                this.$router.push({
                    path: "/waimai_stocks/outstocks/detail",
                    query: {outstock_id: val.outstock_id, criteria: criteria}
                })
            },
            copy(val){
                this.$router.push({path: '/waimai_stocks/outstocks/add', query: {outstock_id: val.outstock_id}})
            },
            remove(val){
                this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
                    ...
                    WdeleteOutstock(val.outstock_id).then((res) => {
            ...
            edit(val){
                ...
                this.$router.push({
                    path: "/waimai_stocks/outstocks/edit",
                    query: {outstock_id: val.outstock_id, criteria: criteria}
                })
            },
            confirm(val){
                this.$confirm('是否确认已完成出库?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    ...
                    WconfirmOutstock(parm).then((res) => {
                        if (res.status_code == 200) {
                            this.$message({
                                type: 'success',
                                message: '确认成功'
            ...
            addInstockOdd(){
                this.$router.replace("/waimai_stocks/outstocks/add")
            },
            reload(){
                this.valueI = [];
                this.valueII = [];
            ...
            getItems() {
                ...
                getWoutstockList(parm).then((res) => {
                    this.listLoading = false;
                    this.total = res.data.totalCount;
                    this.instockList = res.data.outstockList
                })
            }
        }
    }
