                    ...
                    <el-form label-width="120px" :rules="rules" :model="instockForm" ref="instockForm" style="width: 100%;">
                        <el-row>
                            <el-col :span="10">
                                <el-form-item label="出库单号：" prop="instock_bn">
                                    <el-input v-model="instockForm.instock_bn" placeholder="请输入出库编号" class="col"></el-input>
                                </el-form-item>
                            </el-col>
                            <el-col :span="10">
                                <el-form-item label="出库店铺：">
                                    <el-select v-model="instockForm.waimaistore_id" filterable clearable placeholder="请选择出库店铺" @change="getSkuListes(0)" class="col">
                                        <el-option
                                                v-for="item in store"
                        ...
                        <el-row>
                            <el-form-item label="出库类型：">
                                <el-col :span="18">
                                    <el-select v-model="instockForm.type_id" placeholder="请选择出库类型" clearable style="width: 30%;">
                                        <el-option
            ...
            <el-row class='bg'>
                <el-col :span='22' :offset="1">
                    <div style='padding: 10px;padding-left: 0;'>
                        <el-button type="primary" style='width: 150px;' @click='show' v-if="instockForm.waimaistore_id">选择物料</el-button>
                    </div>
                    <el-table
                            emptyText="请选择店铺"
                            :data="tableDataI"
                        ...
                        <el-table-column label="出库数量" width="200">
                            <template slot-scope="props">
                                <el-input-number :min="1" :max="99999999999999" v-model="props.row.nums" style="width: 99%;"></el-input-number>
                            </template>
                        </el-table-column>
...
<script>
    import { getSkuList, getCategoriesList, getWinstockType, addWoutstock, WstoreList, getStock, getPredefined, WoutstockDetail } from '../../api/api'
    export default {
        data () {
            return {
                cat_id: '',
...
//            获取分类列表
            ...
            if (this.$route.query.outstock_id){
                this.bol = true;
        ...
        methods: {
            getback(){
                this.$router.push({psth: "/waimai_stocks/outstocks/lists"})
            },
            subMit(){
                let params = {
                ...
                for (let j in this.tableDataI) {
                    params.items[j] = JSON.stringify({
                        ...
                        num: this.tableDataI[j].nums,
                    })
                }
                addWoutstock(params).then((res)=>{
                    this.$notify.info({
                        title: '消息',
                        message: res.message
                    });
            ...
            getSkuListes(val){
                if (this.bol){
                    this.bol = false;
                    let parm = {per_page: 9999999,store_id: this.instockForm.waimaistore_id,};
                    getStock(parm).then((res)=>{
                        let num = 0;
                        let list = res.data.data;
                        for (let i = 0;i < list.length;i++){
                            for (let j = 0;j < this.tableDataI.length;j++){
                                if (this.tableDataI[j].sku_id == list[i].sku_id){
                                    this.$set(this.tableDataI[j],"stock",list[i].stock)
                                    ++num;
                                }
                            }
                        }
