<template>
    <section class='bg' id="gg">
                    ...
                    <el-table
                        ...
                        <el-table-column label="销售数量" width="200">
                            <template slot-scope="props">
                                <el-input-number :min="1" :max="99999" v-model="props.row.num" style="width: 99%;"></el-input-number>
                            </template>
                        </el-table-column>
        ...
        <el-row class="bg" style='text-align: center;margin: 0;'>
            ...
            <el-button type="primary" class="btnw" @click="getback">取消</el-button>
        </el-row>
    </section>
</template>

<script>
    import { getSkuList, editCommodity, getCategoriesList, getCommoditiesDetail } from '../../api/api'
    export default {
        data () {
            return {
                defaultProps: {
        ...
        created () {
            getCommoditiesDetail(this.$route.query.commodity_id).then(res => {
                this.commodityForm.commodity_title = res.data.commodity_title;
                this.commodityForm.commodity_bn = res.data.commodity_bn;
                this.memo = res.data.memo;
                this.tableDataI = res.data.commodity_skus;
                for (let i in this.tableDataI){
                    this.tableDataI[i].sku_store = this.tableDataI[i].store;
                    switch (this.tableDataI[i].is_gift){
                        case 1:
                            this.tableDataI[i].is_gift = true;
                            break;
                        case 0:
                            this.tableDataI[i].is_gift = false;
                            break;
                    }
                }
                var arr = res.data.platform.split(",");
                for (var i = 0; i < arr.length-1; i++) {
//                    this.commodityForm.platform.push(arr[i] === 'jd' ? '京东' : (arr[i] === 'tm' ? '天猫' : '淘宝'))
                    this.commodityForm.platform.push(arr[i] === 'jd' ? '京东' : (arr[i] === 'tm' ? '天猫' : (arr[i] === "kmjiu" ? "快买酒商城" : '淘宝')))
                }
            })
            .catch(e => {
                console.log(e);
...
//                    parm.platform += (this.commodityForm.platform[i] === '京东' ? 'jd' : this.commodityForm.platform[i] === "天猫" ? "tm" : "tb") + ',';
                ...
                editCommodity(this.$route.query.commodity_id,parm).then((res)=>{
                    if(res.error_code) {
