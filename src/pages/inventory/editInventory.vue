...
<script>
    import { getSkuList, getWareHousesList, getCategoriesList, IncentoryDetail, editInventory } from '../../api/api'
    export default {
        data() {
            return {
                filters: {
        ...
        created () {
            ...
            IncentoryDetail(this.$route.query.inventory_id,parm).then((res)=>{
                ...
                this.outstockForm.inventory_bn = res.data.inventory_bn;
                this.outstockForm.warehouse_id = res.data.warehouse_id;
                this.outstockForm.operator = res.data.operator;
                this.outstockForm.inventory_date = res.data.inventory_date;
                this.outstockForm.stock_deadline = res.data.stock_deadline;
                this.memo = res.data.memo;
                for (let i in this.tableDataI){
                    this.tableDataI[i].sku_store = this.tableDataI[i].num;
        ...
        methods: {
            ...
            subMit(){
                ...
                for (let key in params){
                    if (!params[key]){
                        if (key!="memo"){
                            this.$alert("部分信息未填写,请检查");
                            return;
                        }
                    }
                }
                editInventory(this.$route.query.inventory_id,params).then((res)=>{
                    if (res.error_code){
