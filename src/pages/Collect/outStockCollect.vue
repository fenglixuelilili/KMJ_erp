...
<script>
    import {getOutstockList} from '../../api/api'
    export default{
        data () {
            return {
                tableData: [],
        ...
        methods: {
            educe(){
                window.open(`${this.$store.state.urlBackstage}/outstocks/exportStoreStatistic?access_token=${sessionStorage.getItem('access_token')}&store_name=${this.shop_name}&platform=${this.platform}&startDate=${this.start_date}&endDate=${this.end_date}`)
            },
            getList(){
                var parm = {
